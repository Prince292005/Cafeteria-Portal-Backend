...
public class UserAuthController {
    ...
    public ResponseEntity<String> register(@RequestBody User user) {
        // SECURITY: this is a public, unauthenticated endpoint. Never trust a role
        // supplied by the client here — always force USER. Committee/ADMIN accounts
        // must be granted by promoting an existing user's role directly in the
        // database (or via a future authenticated admin-only endpoint), never through
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        try {
            ...
            String token = jwtUtil.generateToken(userDetails.getUsername(), role, 480); // 8 hours
