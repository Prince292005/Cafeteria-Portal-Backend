...
public class LLMReportServiceImpl implements LLMReportService {
    ...
    private String buildPrompt(MonthlyReportDTO dto) {
        boolean singleCanteen = dto.getCanteenReports() != null && dto.getCanteenReports().size() == 1;
        String scopeLine = singleCanteen
                ? "This report covers a single canteen only — write it as a focused, personal report for that canteen."
                : "This report covers every canteen on campus — write one consolidated report, with a clearly separated section per canteen.";
        ...
        - For each canteen, follow this structure:
            ...
            • Add a "Suggestions" section with 2-4 concrete, actionable recommendations for that canteen based on its specific feedback and complaints this month — be specific, not generic.
