...
public class MonthlyReportServiceImpl implements MonthlyReportService {
    ...
    public MonthlyReportDTO generateMonthlyReport(YearMonth month) {
        return generateMonthlyReport(month, null);
    }

    @Override
    public MonthlyReportDTO generateMonthlyReport(YearMonth month, Long canteenId) {

        String monthName = month.getMonth().name() + " " + month.getYear();
        LocalDateTime end = month.atEndOfMonth().atTime(23, 59, 59);
        LocalDateTime start = month.atDay(1).atStartOfDay();
        List<Canteen> canteens = canteenId == null
