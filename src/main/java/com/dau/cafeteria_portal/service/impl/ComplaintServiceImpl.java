...
public class ComplaintServiceImpl implements ComplaintService {
    ...
    public void updateComplaintStatus(Long complaintId,ComplaintStatus complaintStatus){
        ...
        // Close the loop: let the student know their complaint was resolved
        // instead of leaving them to check back manually. A failed email
        // here should never block the actual status update from saving —
        // the update above has already succeeded by this point.
        if (complaintStatus == ComplaintStatus.RESOLVED
                && complaint.getUser() != null
                && complaint.getUser().getEmailId() != null) {
            try {
                String subject = "Your complaint has been resolved";
                String body = "Hi " + complaint.getUser().getName() + ",<br><br>"
                        + "Good news — the complaint you filed has been marked as <b>resolved</b> by the CMC.<br><br>"
                        + "<b>Title:</b> " + complaint.getTitle() + "<br>"
                        + "<b>Canteen:</b> " + (complaint.getCanteen() != null ? complaint.getCanteen().getCanteenName() : "N/A") + "<br><br>"
                        + "Thank you for helping us keep campus dining accountable.<br><br>"
                ...
                // Notification failure is not fatal to the status update.
                System.err.println("Warning: could not send resolved-notification email: " + e.getMessage());
            }
        }
    }
    @Override
