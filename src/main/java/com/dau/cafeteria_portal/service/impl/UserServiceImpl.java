...
public class UserServiceImpl implements UserService, UserDetailsService {
    ...
    @Override
    public ProfileResponseDTO updateProfile(String emailId, String name, Long mobileNumber) {
        User user = userRepository.findByEmailId(emailId)
                .orElseThrow(() -> new RuntimeException("User not found"));
        ...
        if (mobileNumber != null) {
            // mobileNumber is a unique column — changing it to a number
            // already used by a different account would otherwise fail
            // with an opaque 500 from the DB constraint.
            userRepository.findByMobileNumber(mobileNumber).ifPresent(existing -> {
                if (!existing.getStudentId().equals(user.getStudentId())) {
        ...
        return new ProfileResponseDTO(saved.getStudentId(), saved.getEmailId(), saved.getMobileNumber(), saved.getName());
    }
}
