...
public class QuickFeedbackService {
    ...
    public EngagementResultDTO submit(String emailId, Long canteenId, QuickFeedbackDTO dto) {
        ...
        // Prevent spamming the same canteen with repeated ratings in one day —
        // without this, a single student could submit unlimited ratings and
        // skew that canteen's average rating and rating count.
        LocalDate today = LocalDate.now();
        LocalDateTime startOfDay = today.atStartOfDay();
        LocalDateTime endOfDay = today.plusDays(1).atStartOfDay();
        boolean alreadyRatedToday = quickFeedbackRepo
                .existsByUser_StudentIdAndCanteen_CanteenIdAndCreatedAtBetween(
                        user.getStudentId(), canteenId, startOfDay, endOfDay);
        if (alreadyRatedToday) {
            throw new IllegalStateException("You've already rated this canteen today. Come back tomorrow!");
        }
